import React from 'react';
import Image from 'next/image';

interface PropsType {
    img: string;
    title: string;
    mainTitle: string;
    price: string;
};

const Slide: React.FC<PropsType> = ({ img, title, mainTitle, price }) => {
    return (
        <div className='relative flex justify-center items-center w-full h-[500px]'>
            {/* Text Content */}
            <div className='absolute flex flex-col justify-center items-start left-[30px] md:left-[70px] max-w-[250px] sm:max-w-[350px] 
            space-y-2 lg:space-y-4 bg-[#ffffffa2] sm:bg-transparent 
            p-4 sm:p-0 rounded-lg sm:rounded-none'>
                <h3 className='text-accent text-[24px] lg:text-[28px]'>{title}</h3>
                <h2 className='text-black text-[26px] md:text-[30px] lg:text-[44px] font-bold leading-tight'>
                    {mainTitle}
                </h2>
                <h3 className='text-[24px] text-gray-500'>
                    starting at {" "}
                    <b className='text-[20px] md:text-[24px] lg:text-[30px]'>{price}</b>.00
                </h3>
                <div className='bg-accent text-white text-[14px] md:text-[16px] p-2 px-4 rounded-lg
                inline-block cursor-pointer hover:bg-black'>
                    Shop Now
                </div>
            </div>

            {/* Image */}
            <Image
                className='w-full h-full object-cover rounded-xl'
                src={img}
                alt='banner'
                width={2000}
                height={1000}
                layout='responsive' // Optional: ensures the image is responsive
            />
        </div>
    );
};

export default Slide;


